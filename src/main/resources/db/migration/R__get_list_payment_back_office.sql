CREATE OR REPLACE FUNCTION jaipro.get_list_payment_back_office(p_code_payment text, p_types_ids text, p_last_days integer, p_page integer, p_page_size integer)
 RETURNS TABLE(id integer, operation_code character varying, modality text, method text, type text, issued_by character varying, amount double precision, commission double precision, honorary double precision, creation_date text, status integer, rows integer)
 LANGUAGE plpgsql
AS $function$
declare
