...
begin
    ...
    if (p_message_id is null) then
        return query (select spc.service_proposal_chat_id  as id,
                             spc.message,
                             spc.creation_date,
                             spc.from_specialist,
                             spc.from_customer,
    ...
    select temp1.creation_date,
           temp1.rows
    into
        v_creation_date, v_total_rows
    from (select spc.service_proposal_chat_id,
                 spc.creation_date,
                 CAST(count(*) over () as int) as rows
          from service_proposal_chat spc
          where spc.service_proposal_id = p_service_proposal_id) as temp1
