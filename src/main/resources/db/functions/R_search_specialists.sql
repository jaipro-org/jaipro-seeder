...
CREATE OR REPLACE FUNCTION jaipro.search_specialists(p_id_categories text, p_id_specializations text, p_id_ubigeums text, p_page integer, p_page_size integer)
 RETURNS TABLE(specialist_id uuid, full_name text, photo text, assessment text, about character varying, professions text)
 LANGUAGE plpgsql
AS $function$
begin
	return query(
